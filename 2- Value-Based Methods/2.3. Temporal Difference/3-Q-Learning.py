...
class QLearning():
    ...
    Learns the Q-function by updating using the greedy action,
        ...
        for i in range(self.num_iterations):
            ...
            while not done:
                ...
                Q[curr_state, action] += self.alpha * (
                    R + self.discount * np.max(Q[next_state]) - Q[curr_state, action]
                )
...
qlearn = QLearning(n_iterations, epsilon, discount, alpha)
policy = qlearn.MainAlgorithm()
